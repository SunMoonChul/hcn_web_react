...
const Buttons = () => {
    ...
    const readMore = (item) => {
        ...
        axios.get(`http://localhost:8080/api/account-summaries/${item.name}`)
            .then(response => {
                ...
                <Row key={item.id} className="mb-5 influencer-row">
                    <Col md="3" className="profile-section">
                        <div className="d-flex align-items-center">
                            ...
                            <div className="item-info ml-3">
                                <h5>{item.name}</h5>
                                <p>{item.category}</p>
                                <Button className="details-button" onClick={() => readMore(item)}>자세히 보기</Button>
                                {isModalOpen && (
                                    <ModalComponent
                                        isOpen={isModalOpen}
                    ...
                    <Col md="6" className="stats-section">
                        ...
                        <div className="item-images mt-3 d-flex justify-content-center">
                            {item.posts.slice(0, 6).map((post, i) => (
            ...
            {currentItem && currentItem.image_urls && currentItem.image_urls.length > 0 && (
                <Modal isOpen={modal} toggle={toggle} className="click_img" onClick={handleModalClick}>
                    <ModalBody className="text-center">
                        ...
                        />
                        <div className="info">
                            <p>좋아요: {currentItem.likes.toLocaleString()}&nbsp;&nbsp;댓글: {currentItem.comments.length.toLocaleString()}&nbsp;&nbsp;게시 날짜: {new Date(currentItem.post_time).toLocaleDateString()}</p>
                        </div>
                    </ModalBody>
                </Modal>
            )}
